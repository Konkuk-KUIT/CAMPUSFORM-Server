...
public class RecruitingStageService {
    ...
    public ProjectResponse completeDocument(Long projectId, Long userId) {
        ...
        if (holdCount > 0) {
            throw new IllegalStateException(
                    "서류 단계를 마감할 수 없습니다. 서류 심사가 보류(HOLD)인 지원자가 " + holdCount + "명 있습니다. 모두 합격/불합격 처리 후 마감해 주세요.");
        }
    ...
    public ProjectResponse completeAll(Long projectId, Long userId) {
        ...
        if (holdCount > 0) {
            throw new IllegalStateException(
                    "면접 단계(프로젝트 전체)를 종료할 수 없습니다. 면접 결과가 보류(HOLD)인 지원자가 " + holdCount + "명 있습니다. 모두 합격/불합격 처리 후 종료해 주세요.");
        }
